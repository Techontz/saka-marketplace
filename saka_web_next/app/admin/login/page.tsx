...
import { AuthShell } from "@/components/admin/layout/AuthShell";
import { Button, Checkbox, Field, FormError, Input } from "@/components/admin/ui";
...
function LoginForm() {
  ...
  const destination =
    nextParam &&
    /^\/(?!\/)[\w\-./?=&%]*$/.test(nextParam) &&
    // Must land INSIDE the admin portal — see the vendor login page for why
    // same-origin stopped being a sufficient test once the three apps moved
    // onto one domain.
    (nextParam === "/admin" || nextParam.startsWith("/admin/") || nextParam.startsWith("/admin?"))
      ? nextParam
      : "/admin";
