...
type ApiUser = {
  ...
  /** Publishing is gated on a verified phone. Shown on the account screen. */
...
/*
 * The marketplace has NO role gate at all.
 *
 * Anyone with an account is a customer here — including admins and vendors,
 * who browse and save listings like everyone else. The API remains the
 * authority on what each account may do.
 */
...
async function handleLogin(httpRequest: NextRequest): Promise<Response> {
  ...
  try {
     ...
     * authenticated, so the API omits `permissions`. One extra server-side
     * request makes this endpoint's contract match `/api/auth/session`
     * exactly, which is what lets the header render the same way after a
     * sign-in as after a refresh.
     */
...
/**
 * Registration signs the new customer straight in.
 *
