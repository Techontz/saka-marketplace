...
const TONE_CLASSES: Record<NonNullable<MapPin["tone"]>, string> = {
  listing: "bg-teal text-white",
  business: "bg-navy text-white",
  place: "bg-orange text-white",
};
...
export function MapView({
  ...
  const controlClass =
    "flex h-9 w-9 items-center justify-center rounded-lg bg-white text-navy shadow-md transition hover:bg-teal hover:text-white disabled:cursor-not-allowed disabled:opacity-40";

  return (
    <div
      className={
        fullscreen
          ? "fixed inset-0 z-[60] bg-navy/95 p-3 sm:p-6"
          : "relative"
      }
    >
      <div
        ref={containerRef}
        className={`relative overflow-hidden rounded-xl border border-border bg-[#EEF4FF] select-none ${
      ...
      >
        ...
        {shapes.map((shape) =>
          shape.label && shape.anchor ? (
            <span
              key={`${shape.id}-label`}
              className="pointer-events-none absolute z-[6] -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-bold text-navy shadow"
        ...
        {draw &&
          drawPath?.points.map((point, index) => (
            <button
              ...
              className="absolute z-10 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-teal shadow-md transition hover:scale-125 hover:bg-orange"
        ...
        {clusters.map((cluster) => {
          ...
          return (
            <button
              ...
              } hover:scale-110 ${isCluster ? "bg-navy text-white" : tone} ${
        ...
        {openPin && openPinPosition && (
          <div
            className="absolute z-30 w-56 -translate-x-1/2 animate-scale-in-soft rounded-xl border border-border bg-white p-3 shadow-xl"
            style={{
          ...
          >
            <button
              ...
              aria-label="Close"
              className="absolute right-2 top-2 text-muted-foreground transition hover:text-navy"
            >
            ...
            {openPin.image && (
              <SafeImage
                ...
                fallbackClassName="mb-2 h-24 w-full rounded-lg"
              />
            )}

            <p className="pr-4 text-sm font-bold leading-snug text-navy line-clamp-2">
              {openPin.label}
            </p>

            {openPin.meta && (
              <p className="mt-0.5 text-xs text-muted-foreground line-clamp-1">{openPin.meta}</p>
            )}

            {openPin.sublabel && (
              <p className="mt-1 text-sm font-extrabold text-teal">{openPin.sublabel}</p>
            )}
            ...
            <div className="mt-2.5 flex items-center gap-2">
              {openPin.href && (
                <Link
                  href={openPin.href}
                  className="flex-1 rounded-full bg-teal px-3 py-1.5 text-center text-xs font-semibold text-white transition hover:opacity-90"
                >
                  View
                </Link>
              )}
              <a
                href={googleDirectionsUrl(openPin.lat, openPin.lng)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-1.5 text-xs font-semibold text-navy transition hover:border-teal hover:text-teal"
              >
        ...
        {interactive && allowLayers && (
          <div
            className="absolute left-3 bottom-6 z-20 flex overflow-hidden rounded-lg bg-white shadow-md"
            onPointerDown={(event) => event.stopPropagation()}
            onClick={(event) => event.stopPropagation()}
          >
            {(Object.keys(MAP_LAYERS) as MapLayerId[]).map((id) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold transition ${
                  layerId === id ? "bg-teal text-white" : "text-navy hover:bg-teal/10"
                }`}
              >
                {MAP_LAYERS[id].label}
              </button>
          ...
          >
            ...
            {allowMeasure && (
              <button
                ...
                className={`${controlClass} ${measuring ? "bg-orange text-white" : ""}`}
              >
        ...
        {draw && (
          <div
            className="absolute left-3 top-3 z-20 rounded-xl bg-white/95 px-3 py-2 shadow-md"
            onPointerDown={(event) => event.stopPropagation()}
            onClick={(event) => event.stopPropagation()}
          >
            {drawMetrics ? (
              <>
                <p className="text-sm font-extrabold text-navy">{formatArea(drawMetrics.area)}</p>
            ...
            <div className="mt-2 flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold text-navy transition hover:border-teal hover:text-teal disabled:opacity-40"
              >
              ...
              <button
                ...
                disabled={!drawing?.length}
                className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold text-navy transition hover:border-orange hover:text-orange disabled:opacity-40"
              >
          ...
          >
            ...
            {measurePoints.length > 0 && (
              <button
                ...
                className="mt-2 inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold text-navy transition hover:border-orange hover:text-orange"
              >
                <Trash2 className="h-3 w-3" />
                Reset
              </button>
        ...
        {onAreaSearch && !autoSearch && moved && !draw && !measuring && (
          <button
            ...
            className="absolute left-1/2 top-3 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full bg-navy px-4 py-2 text-sm font-semibold text-white shadow-lg transition hover:opacity-90"
          >
        ...
        {loading && !draw && (
          <div className="absolute left-1/2 top-3 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-navy shadow-md">
        ...
        {geo.error && (
          <p className="absolute bottom-8 right-3 z-20 max-w-xs rounded-lg bg-white/95 px-3 py-2 text-xs text-muted-foreground shadow">
            {geo.error}
          </p>
        )}

        <p className="pointer-events-none absolute bottom-0 right-0 z-20 bg-white/80 px-2 py-0.5 text-[10px] text-muted-foreground">
          {layer.attribution}
        </p>
      </div>
    </div>
