...
import { MapView } from "@/components/vendor/maps/MapView";
import { MAP_LAYERS, type MapLayerId } from "@/lib/vendor/map";
import {
  formatArea,
    ...
    <div className={className}>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-soft">
          <span className={isClosed ? "font-semibold text-ink" : undefined}>{readout}</span>
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full ring-1 ring-line border-0 border-line px-3 py-1.5 text-xs font-semibold text-ink transition hover:border-brand hover:text-brand-ink disabled:opacity-40"
          >
          ...
          {!readonly && (
            <>
              ...
              <button
                ...
                onClick={() => emit([])}
                disabled={points.length === 0}
                className="inline-flex items-center gap-1.5 rounded-full ring-1 ring-line border-0 border-line px-3 py-1.5 text-xs font-semibold text-ink transition hover:border-danger hover:text-danger disabled:opacity-40"
              >
      ...
      {!readonly && (
        <p className="mt-2 flex items-start gap-1.5 text-xs text-ink-soft">
          <Crosshair className="mt-0.5 h-3.5 w-3.5 shrink-0 text-brand-ink" />
