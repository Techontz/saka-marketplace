...
import { ApiError } from "@/lib/vendor/api/errors";
import { cn } from "@/lib/vendor/cn";
import { formatCount } from "@/lib/vendor/format";

/**
 * The vendor portal's UI primitives.
 *
 * Everything a screen needs to render the four states every list has — loading,
 * empty, error, populated — lives here, so no screen invents its own. The
 * failure mode of a portal built page-by-page is that half the screens forget
 * the empty state and show a blank panel that looks broken — and for a vendor,
 * "broken" and "I have no listings yet" must never look the same.
 */
...
export function PageHeader({
  ...
  title: string;
  /** ReactNode, not string — headers routinely need a badge beside the text. */
