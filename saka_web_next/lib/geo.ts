 ...
 * Shared map geometry.
 *
 * Web Mercator projection plus the measurements the map draws on top of it.
 * Pulled out of MapView because the boundary editor, the parcel panel and the
 * measure tool all need the same numbers, and three copies of a haversine is
 * three chances for them to disagree about how big a plot is.
 *
 * The area formula MIRRORS the server's LandBoundaryService: same mean Earth
 * radius, same spherical-excess method. That is deliberate — the seller sees a
 * live area while dragging a corner, and it would be indefensible for that
 * number to change the moment they pressed Save.
 */
