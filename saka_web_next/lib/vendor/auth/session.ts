...
const baseCookieOptions = {
   ...
   * "/" path would attach the vendor session to every storefront request
   * too — a token travelling somewhere it is never read. All vendor pages
   * and its API proxy live under /vendor, so the narrower path costs
...
export async function clearSession(): Promise<void> {
  ...
  for (const name of [TOKEN_COOKIE, EXPIRY_COOKIE, SESSION_FLAG_COOKIE]) {
    ...
    // "/vendor". Without this, signing out clears the server-side token
