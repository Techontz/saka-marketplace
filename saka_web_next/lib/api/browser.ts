 ...
 * httpOnly cookie. See `app/api/saka/[...path]/route.ts`.
 */

const PROXY_BASE = "/api/saka";
...
export function authRequest<T>(action: AuthAction, body?: unknown): Promise<T> {
  return request<T>(`/api/auth/${action}`, {
