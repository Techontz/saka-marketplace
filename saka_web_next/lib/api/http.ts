 ...
 *
 * The browser therefore never holds the access token.
 */
...
export async function request<T>(url: string, options: RequestOptions = {}): Promise<T> {
  ...
  // Default to no-store — account data is per-user and must never be served
  // from a shared cache. Public marketplace reads pass an explicit `cache`
  // instead, which is what lets listing pages be statically revalidated.
