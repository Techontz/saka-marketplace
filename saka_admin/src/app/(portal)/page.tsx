...
import { Badge, Card, ErrorState, PageHeader, TableSkeleton, humanise } from "@/components/ui";
...
function Stats({ data }: { data: Overview }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat label="Total users" value={data.users.total} hint={`${formatCount(data.users.active)} active`} href="/users" />
        <Stat label="Vendors" value={data.users.vendors} hint={`${formatCount(data.verifications.pending)} awaiting review`} href="/vendors" />
        <Stat label="Listings" value={data.listings.total} hint={`${formatCount(data.listings.published)} published`} href="/listings" />
        <Stat
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat label="Featured" value={data.listings.featured} href="/listings?featured=1" />
        <Stat
          label="Rejected"
        ...
        />
        <Stat label="Reviews" value={data.engagement.reviews} hint={`${formatCount(data.engagement.pending_reviews)} pending`} href="/reviews" />
        <Stat label="Inquiries" value={data.engagement.inquiries} hint={`${formatCount(data.engagement.unread_inquiries)} unread`} />
      </div>

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat label="Categories" value={data.catalog.categories} href="/categories" />
        <Stat label="Public places" value={data.catalog.public_places} href="/places" />
